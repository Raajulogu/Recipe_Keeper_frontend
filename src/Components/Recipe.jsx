...
const Recipe = ({ data, setData, ind, setInd, theme, setTheme }) => {
  ...
  async function handleComment() {
    ...
    let res = await fetch(
      `https://recipe-keeper-backend.vercel.app/api/recipe/comment/${data[ind]._id}`,
      {
        method: "PUT",
  ...
  return (
    <Base theme={theme} setTheme={setTheme}>
      <div className="recipe-container" style={{ color: `${fontColor}` }}>
        ...
        <div className="recipe-body">
          ...
          <ul className="ingrediants-list">
            {data[ind].ingredients.length &&
              data[ind].ingredients.map((rec, index) => (
                <li key={index}>{rec}</li>
              ))}
          </ul>
          <h4>Instructions:</h4>
          <p className="recipe-intsrut">
            &emsp; &emsp; {data[ind].instructions}
          </p>
        </div>
        <br />
        ...
        <div className="comments">
          <h3>Comments:</h3>
          {data[ind].comments.length ? (
            data[ind].comments.map((com, index) => (
